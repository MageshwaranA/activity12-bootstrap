...
```{r}
ggplot(gss,aes(x=educ,y = ..density..))+
  geom_density()+
...
```{r}
answer <- t.test(gss$educ,conf.level = .90)
answer
```
...
```{r}
repl <- replicate(1000,sample(),simplify = FALSE)
```
...
```{r}
do.call("rbind",tapply(boot_educ$replicate, boot_educ$educ, quantile))
